...
// create-checkout-session v30 (2026-09-24, Shop v2 Phase 0):
//   * Prices, labels, print sizes and shipping countries now come from public.print_products.
//     Seeded to match v29 exactly, so buyers see no change. If the table can't be read, the
//     constants below are used instead (they are the v29 values, kept as the fallback).
//   * Inactive products are refused (the two dead framed SKUs, which Prodigi can't make).
//   * Allowed countries = the countries EVERY item in the cart can ship to.
//   * metadata[kind]=print so the webhook can tell print checkouts from preset/gallery ones.
// Rollback: redeploy _edge_fn_snapshots/create-checkout-session.v29.ts with verify_jwt:false.
...
function sizeAllowed(
  ...
  dimsIn?: { shortIn: number; longIn: number; ratio: number; minDpi?: number },
): { ok: boolean; reason?: string } {
  const dims: { shortIn: number; longIn: number; ratio: number; minDpi?: number } | undefined = dimsIn ?? SKU_PRINT_DIMS[skuUpper];
  if (!dims) return { ok: true };
  const neededLongPx = dims.longIn * (dims.minDpi ?? MIN_PRINT_DPI);
...
type Product = { sku: string; label: string; price: number; dims: { shortIn: number; longIn: number; ratio: number; minDpi: number }; countries: string[] };

// Active print products from the catalog table, keyed by SKU. null = table unreadable,
// in which case the caller uses the v29 constants.
// deno-lint-ignore no-explicit-any
async function loadProducts(supabase: any): Promise<Record<string, Product | false> | null> {
  try {
    const { data, error } = await supabase.from("print_products")
      .select("sku,label,price_cents,short_in,long_in,ratio,min_dpi,countries,active,channel");
    if (error || !data || !data.length) { console.error("print_products unavailable, using constants:", error?.message ?? "empty"); return null; }
    const out: Record<string, Product | false> = {};
    for (const r of data) {
      if (!r.active || r.channel === "client") { out[r.sku] = false; continue; }
      out[r.sku] = {
        sku: r.sku, label: r.label, price: r.price_cents,
        dims: { shortIn: Number(r.short_in), longIn: Number(r.long_in), ratio: Number(r.ratio), minDpi: r.min_dpi ?? MIN_PRINT_DPI },
        countries: r.countries ?? [],
      };
    }
    return out;
  } catch (e) { console.error("loadProducts threw:", String(e)); return null; }
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const raw of rawItems) {
      ...
      const product = products ? products[skuUpper] : undefined;
      if (products && product === false) return json({ error: `This size is no longer available: ${raw.sku}` }, 400);
      const unitPrice = product ? product.price : (products ? undefined : PRICES_CENTS[skuUpper]);
      if (!unitPrice) return json({ error: `Unknown SKU: ${raw.sku}` }, 400);
      if (product) countrySets.push(product.countries);
      ...
      const gate = sizeAllowed(skuUpper, long, short, product ? product.dims : undefined);
      if (!gate.ok) {
      ...
      const label = (product ? product.label : SKU_LABELS[skuUpper]) ?? skuUpper;
    ...
    // Every item must be able to ship to the address, so offer only the countries all items
    // share (table path). Fallback path: v29 rule, one framed item restricts the whole order.
    let allowedCountries = anyFramed
      ? PAPER_COUNTRIES.filter((c) => FRAMED_COUNTRIES.includes(c))
      : PAPER_COUNTRIES;
    if (products && countrySets.length === clean.length) {
      allowedCountries = countrySets.reduce((acc, set) => acc.filter((c) => set.includes(c)));
    }
    if (!allowedCountries.length) return json({ error: "These items can't ship together to any one country. Try ordering them separately." }, 422);
    ...
    params.append("metadata[kind]", "print");
